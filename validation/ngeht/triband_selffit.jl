...
#         [--resume output/<tag>_params.csv] [--single-stage 0] [--eta-end 0] [--polish 0] [--solve 40] [--lambda 0.01] [--probes 8] [--probe-every 4] [--dense 0] [--chunk 8]
...
npolish = getopt("--polish", 0); nsolve = getopt("--solve", 40); λ0 = getopt("--lambda", 0.01); nprobes = getopt("--probes", 8); probe_every = getopt("--probe-every", 4); ndense = getopt("--dense", 0); chunk = getopt("--chunk", 8)
