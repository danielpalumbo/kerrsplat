...
#                                                  [--prune 0.02] [--densify 0] [--max 600] [--seed 1] [--tag shell]
...
clean = polarized_cube(cpu, p, times, [ν], L; nmax, slab)
...
open(joinpath(outdir, "shell_$(tag)_summary.txt"), "w") do io
    println(io, "large-N self-fit: $(res)² pixels, fov $fov M, $N samples, nmax $nmax slab $slab, $nframes frames, $ndat values; shell of $n parcels of $scale M, $iterations iterations, eta $η, hygiene every $every (prune $prune, densify $densify, max $maxsplats), seed $seed")
