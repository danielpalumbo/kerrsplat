...
function timed_stages(cache, a, θo)
    ...
    t_rec = time() - t0
    return (k0 = t_k0, sort = t_sort, k1 = t_k1, k2 = t_k2, rec = t_rec)
end
...
for (name, backend) in (("CUDA", CUDABackend()), ("CPU ($(Threads.nthreads()) threads)", CPU()))
    ...
    @printf("[%s]   K0 root cases %.4f s | sort %.4f s | K1 constants %.4f s (%.1f µs/px) | K2 direct march %.3f s (%.1f ns/sample) | K2 recurrence (r, θ; anchor 64) %.4f s (%.2f ns/sample)\n",
            name, s.k0, s.sort, s.k1, s.k1 / npix * 1e6, s.k2, s.k2 / (npix * N) * 1e9, s.rec, s.rec / (npix * N) * 1e9)
