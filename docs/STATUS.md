...
| Uniqueness table, frames and frequency rows (160 iterations) | `Fit` | (experiment) two frames instead of six: the profile no longer separates 0.9 from 0.98 and the inclination stays 6° off; a second frequency: Δχ² 9,500 for +0.08 (8,600 baseline), its gain on the plasma side | the motion carries the spin |
| Precision converter (`Geodesics.precision`) and the FP32-clean transport | `Geodesics`, `Transfer` | the Float32 tails image and dual-sweep gradient over Float64 geodesics agree with Float64 to 3e-7 of the peak and 1.5e-6 of the largest entry (CPU and CUDA); seven mechanisms fixed with Float64 unchanged, the step operator bit for bit (`docs/notes/2026-09-12_fp32_transport.md`) | `test_precision`, the step's BigFloat gate, the Gold fluxes |
| FP32 timing (`validation/large_n/bench_precision.jl`, the tails and dual sweep of four batched frames over lists, RTX 2080 SUPER) and the fit in Float32 (`shell_selffit.jl --precision Float32`) | `Splats`, `Fit` | (measurement) Float32 9.0× Float64 at 64² × 160 × 300 (6.37 → 0.71 s), 5.9× at 128² × 160 × 600, gradients to 7e-6 and 3e-6, finite after mechanisms 7–9; the 300-parcel shell fit 1901 → 303 s (6.3×) at the same χ²/N (1.063 vs 1.064) with the fields within the run-to-run spread (`docs/notes/2026-09-12_fp32_transport.md`) | Float64 against Float32 on the card, the Float64 fit as the baseline |
| Large N with the spacetime free (`joint_selffit.jl --shell 300 --hygiene 25`, Keplerian priors, from the true spin and 10° off in inclination) | `Fit` | (experiment) χ²/N 5.03 → 1.036 (truth 1.003), 300 → 71 parcels, spin 0.918, inclination 55.7° (from 50°, truth 60°) in 240 iterations | the first end-to-end instance of the program |
| Spin profile from the shell start (`joint_selffit.jl --shell 300 --hygiene 25 --fix-spin` at 0.7, 0.9, 0.98) | `Fit` | (experiment) minimum at the truth (χ²/N 1.019, truth 1.003, inclination to 0.6°), Δχ² 2,230 for +0.08 and 3,540 for −0.2 of spin on 65,536 values: four to six tenths of the correct model's discrimination per value, the spin determined with no knowledge of the number of parcels (`docs/notes/2026-09-10_large_n.md`) | three two-hour GPU fits |
| Two-band large-N self-fit (`shell_selffit.jl --n 300 --frequencies 230,345`, the true spacetime) | `Fit` | (experiment) χ²/N 6.21 → 1.068 (truth 1.003), 300 → 31 parcels; density PSNR 23.7 dB (one band 22.4), temperature error 0.138 (one band 0.20–0.22), field error 0.167 (one band 0.14–0.15): the second band lifts the temperature side of the degeneracy, not the field (`docs/notes/2026-09-10_large_n.md`) | a 67-minute GPU fit |
| The triband ngEHT self-fit (`validation/ngeht/make_campaign.py` with ngehtsim's reference array, `triband_selffit.jl`, `viz/triband_movie.jl`; the batched time-resolved likelihood with the coverage's own noise, Float32 scans and the analytic visibility seed; `fit_joint!` on `BandScans` for the spacetime free) | `Fit`, `Splats` | (experiment) the full run (64² × 160, 300 parcels, 300 iterations, 241,264 values, 5 h in Float32): χ²/N 33,700 → 31.6 (truth 1.0, still descending), density 22.1 dB, temperature error 0.11, field 0.16, as the image-domain fits recover them; the fields are carried by the campaign's three bands, the χ² floor is the schedule's distance (`docs/notes/2026-09-14_ngeht_triband.md`) | `test_timeresolved`, `test_joint_scans`, the CPU smokes in both precisions and with the spacetime free |
| Large-N self-fits from over-complete shells (`validation/large_n/shell_selffit.jl`: 300 and 600 parcels, hygiene, the true spacetime) | `Fit` | (experiment) χ²/N → 1.066 and 1.045 (truth 1.003), 300 → 26 and 600 → 35 parcels; both give density PSNR 22.4 dB; the two fits agree with each other (density 0.087, temperature 0.12, field 0.09) closer than with the truth (0.076, 0.20–0.22, 0.14–0.15) | the fields are start-independent; the plasma departure is the one-band degeneracy |
