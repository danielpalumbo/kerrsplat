...
| Multifrequency n ≤ 1 self-fit (`--frequencies 86,345`, per-band noise; note section "Multifrequency"): bands bracketing the parcels' turnover (178 GHz) against 230 GHz alone | `Fit` | the single-band fit; marginal Fisher errors from the finite-difference audit | 1000 iterations from one start: three of four parcels to Θe 0.04–1.4% and B 0.07–2% (single band: 5–18% and 2.5–7%), densities 2–3× better; Fisher σ(ln Θe) 13–20× and σ(ln B) 8–12× tighter, σ(ln nₑ) 2×; the third band adds little; one edge parcel stays in the nₑ–B valley in every band set (optimizer residual, Δχ² 10³ above the truth) |
