...
| The triband ngEHT self-fit (`validation/ngeht/make_campaign.py` with ngehtsim's reference array, `triband_selffit.jl`, `viz/triband_movie.jl`; the batched time-resolved likelihood with the coverage's own noise, Float32 scans and the analytic visibility seed; `fit_joint!` on `BandScans` for the spacetime free) | `Fit`, `Splats` | (experiment, in progress) the first full run (64² × 160, 300 parcels, 300 iterations, 241,264 values, 5 h in Float32) ended at χ²/N 31.6 still descending, unconverged; resumed for 600 more iterations at a lower step, whose χ²/N (expected ≈ 1, with a plasma distribution that need not be the truth's) is the result (`docs/notes/2026-09-14_ngeht_triband.md`) | `test_timeresolved`, `test_joint_scans`, the CPU smokes |
