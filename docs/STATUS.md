...
| Multifrequency n ≤ 1 self-fit (`--frequencies 86,345`, per-band noise; note section "Multifrequency"): bands bracketing the parcels' turnover (178 GHz) against 230 GHz alone | `Fit` | the single-band fit; marginal Fisher errors from the finite-difference audit | Θe pinned to ~1% and B to a few per cent (single band: up to 16% and 14%); Fisher σ(ln Θe) 13–20× and σ(ln B) 8–12× tighter; nₑ only 2× (MF1000) |
