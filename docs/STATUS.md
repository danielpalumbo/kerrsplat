...
| The triband ngEHT self-fit (`validation/ngeht/make_campaign.py` with ngehtsim's reference array, `triband_selffit.jl`, `viz/triband_movie.jl`; the batched time-resolved likelihood with the coverage's own noise, Float32 scans and the analytic visibility seed; `fit_joint!` on `BandScans` for the spacetime free) | `Fit`, `Splats` | (experiment, unconverged) 64² × 160, 300 parcels: χ²/N 33,700 → 31.6 in 300 iterations, → 7.44 in 600 more (9.4 / 5.9 / 2.7 per band, truth 1.0), stalled at Adam's closing step with hygiene events costing an order of magnitude each; a Gauss–Newton polish on the card is the next tool (`docs/notes/2026-09-14_ngeht_triband.md`) | `test_timeresolved`, `test_joint_scans`, the CPU smokes |
