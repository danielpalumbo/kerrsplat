...
| Precision converter (`Geodesics.precision`) and the FP32 transport probe | `Geodesics` | (finding) the tails image in Float32 over Float64 geodesics agrees with Float64 to 1.1e-4 of the peak, but the coefficient chain promotes to Float64 and the invariants hit Float32 subnormals: not a drop-in (`docs/notes/2026-09-12_fp32_transport.md`) | `test_precision` |
