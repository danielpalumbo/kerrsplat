...
| The triband ngEHT self-fit (`validation/ngeht/make_campaign.py` with ngehtsim's reference array, `triband_selffit.jl`, `viz/triband_movie.jl`; the batched time-resolved likelihood with the coverage's own noise and Float32 scans) | `Fit`, `Splats` | (experiment) the Float64 pilot at 48² with 200 parcels recovers the fields as well as the image-domain shell fits (density 21 dB, temperature 0.12, field 0.18) while the χ² stays 150× the floor at 1 mJy sensitivity; the full Float32 run at 64² × 160 with 300 parcels is in `docs/notes/2026-09-14_ngeht_triband.md` | `test_timeresolved` (batched = per-frame to 1e-12, Float32 gradient to 1e-3), the CPU smoke in both precisions |
