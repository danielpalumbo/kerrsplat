...
@testset "KerrSplat CI subset" begin
    ...
    test_large_n_init(; res = 8, N = 12, n = 40, iterations = 12, prune_fraction = 0.2)
    test_frame_batching(CPU(); res = 6, N = 16, tol = 1e-13, label = "CPU backend")
    test_precision(; res = 6, N = 16)
    test_directional_image(CPU(); res = 6, N = 16)
    test_joint_fit(CPU(); res = 6, N = 16, iterations = 30, tol = 5e-5, θtol = deg2rad(4.0), atol = 0.2, label = "CPU backend")
    test_joint_scans(CPU(); res = 6, N = 16, label = "CPU backend")
    test_gauss_newton(CPU(); res = 6, N = 16, label = "CPU backend")
    test_gain_scans(CPU(); res = 6, N = 16, label = "CPU backend")
