...
@testset "KerrSplat" begin
    ...
    @testset "Geodesics on the CPU backend" begin
        ...
        test_ray_lists(CPU(); res = 8, N = 40, tol = 1e-12, label = "CPU backend")
        test_large_n_init(; res = 12, N = 16, n = 60, iterations = 12)
        test_frame_batching(CPU(); res = 8, N = 40, tol = 1e-13, label = "CPU backend")
        ...
        test_joint_fit(CPU(); res = 8, N = 40, iterations = 60)
    ...
    if CUDA.functional()
        ...
        @testset "Geodesics on CUDA" begin
            ...
            test_ray_lists(CUDABackend(); res = 32, N = 100, tol = 1e-12, label = "CUDA")
            test_frame_batching(CUDABackend(); res = 32, N = 100, tol = 1e-13, label = "CUDA")
            test_joint_fit(CUDABackend(); res = 16, N = 60, iterations = 40, label = "CUDA")
