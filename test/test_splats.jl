...
function test_splats(backend; res::Int, N::Int, label::String)
    ...
    @testset "$label splats $(res)² × $N" begin
        ...
        copyto!(params, p)
        img = Array(render(cache, params, t_obs))
...
function thin_image_march(p, cache::GeodesicCache{T,N}, t_obs) where {T,N}
    ...
    r = cache.ranges
    for (case, rng) in ((Case2(), r.case2), (Case3(), r.case3), (Case4(), r.case4))
        for j in rng
            acc, _, _ = march_ray(c, zero(eltype(p)), pcs, j, met, cache.θo, case, Val(N), Val(64))
            img[j] = acc
        end
    end
    return img
...
function test_splat_gradients(; res::Int, N::Int, tol::Float64)
    ...
    @testset "Enzyme reverse gradient of an image loss, $(res)² × $N, $(length(p)) parameters" begin
        ...
        @test L0 > 0
        t0 = time()
        g = Enzyme.gradient(Enzyme.Reverse, loss, p_start)[1]
        t_first = time() - t0
        t0 = time()
        g = Enzyme.gradient(Enzyme.Reverse, loss, p_start)[1]
        t_grad = time() - t0
